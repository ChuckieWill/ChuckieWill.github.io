...
int main() {
	...
	//int& ref = 10;  引用本身需要一个合法的内存空间(堆区或栈区)，因此这行错误
	//加入const就可以了，编译器优化代码，int temp = 10; const int& ref = temp;
	const int& ref = 10; //10 是常量，在常量区
...
int main() {
    ...
    func1(10);//占位参数有默认参数，不用传  效果同func(10,10)
...
void test02() {
	...
	//Person(p4);   Person(p4) 等价于 Person p4
}
...
void test01()
{
	...
	Person p2(10);
	p2.PersonAddPerson(p1).PersonAddPerson(p1).PersonAddPerson(p1); //以引用的形式返回对象本体就可以实现链式编程
...
void test01()
{
	...
	p->ShowPerson();  //但是如果成员函数中用到了this指针，就不可以了   此时函数内的this指针就等于p指针 而p指针为空 即this指针为空 则访问不到成员变量 自然报错
}
...
* 声明对象前加const称该对象为常对象
* 常对象只能调用常函数（常对象不能修改成员变量 而成员函数可以修改成员变量  所以不能调用  常函数不能修改成员变量 所以可以调用）
...
* 利用虚继承可以解决菱形继承问题
  * 虚继承可以使数据只有一份，通过对象本身或父类作用域修改和获取其实是操作的同一个地址的数据
...
//纯虚析构函数必须实现，否则报错，因为有可能有堆区空间需要在析构函数中释放
Animal::~Animal()
{
...
void test01()
{
	...
	delete animal;  //父类提供虚析构函数后，此操作会先调用子类的析构函数，再调用f
}
...
​	1. 虚析构或纯虚析构就是用来解决通过父类指针释放子类对象（可以调用子类的析构函数，释放其在堆区的空间）

​	2. 如果子类中没有堆区数据，可以不写为虚析构或纯虚析构

​	3. 拥有纯虚析构函数的类也属于抽象类（抽象类不能实例化）
