...
```c++
        // f[i][j] 记录s的前i个元素是否可以用p的前j个元素匹配 bool
        // f[i][j] = f[i-1][j-1]  s[i-1] == p[j-1] || p[j-1] == '.'
        // f[i][j] = f[i][j-2]  p[j-1] == '*'  不使用*
        // f[i][j] = f[i-1][j]  p[j-1] == '*' && ( p[j-2] == '.' || p[i-2] == s[i-1])
        //      使用*，但是.*是可以匹配多个的，所以可能也会给是s[i-2]用，所以等于f[i-1][j]
        //      如果s[i-2]也匹配了*,那就是f[i-1][j]， 如果不匹配，那就是对于s[i-2]，*匹配0次，即f[i-1][j] = f[i-1][j-2]
        // 初始化
        // f[0][0] = true
        // f[1-n][0] = false
        // f[0][1-m] = f[0][j-2] && p[j-1] == '*'
```

* 72[编辑距离](https://leetcode.cn/problems/edit-distance/description/)

```c++
        // f[i][j] 记录w1转换为w2的最少操作次数
        // f[i][j] = f[i-1][j-1]   w1[i-1] == w2[j-1]
        // f[i][j] = f[i-1][j-1] + 1  w1[i-1] != w2[j-1] 替换
        // f[i][j] = f[i][j-1] + 1  w1[i-1] != w2[j-1] 插入  插入的元素和w2[j-1]匹配，则需要w1[i-1]与w2[j-2]匹配即f[i][j-1]
        // f[i][j] = f[i-1][j] + 1  w1[i-1] != w2[j-1] 删除  删除w1[i-1], 则需要w1[i-2]与w2[j-1]匹配即f[i-1][j]
