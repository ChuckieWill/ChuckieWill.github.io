...
* 416 [分割等和子集](https://leetcode.cn/problems/partition-equal-subset-sum/)
  ...
  * 起始第一行为false, f[0,0] = true
  * 空间优化
* 322 [零钱兑换](https://leetcode.cn/problems/coin-change/)
  * `F[i] = min(1+F[i-coins[j]])  i>=0 i <=amount  0<=j<coins.size()`
  * `F[0] = 0`
...
// 初始化都为1 因为自己本身就为1个
// 注意+1时灵活处理，因为if(nums[j] < nums[i])可能导致进入不了max(F[j]+1) 从而没有+1, 但这个元素本身也是一个子序列 （前面元素都大于等于当前元素的情况）
```
