...
> [视频教程](https://www.bilibili.com/video/BV1ap421d7Hf/?spm_id_from=333.999.0.0&vd_source=7230a052308bbb41976f248d2c778e3a)
>
> https://github.com/chenzomi12/AISystem
>
> [美团外卖向量检索](https://tech.meituan.com/2024/04/11/gpu-vector-retrieval-system-practice.html)
...
* Quantized
  * 量化索引 Quantizeg indexing
    ...
    * 标量量化 SQ：将向量对称划分包含每个维度的最小值和最大值的容器，将向量中的浮点数转换为整数。e.g.，神经网络模型对权重参数的量化。
...
>
> 非常直观的教程:https://blog.csdn.net/huangfei711/article/details/78480078

1. 随机选k个点
2. 计算每个点i到这k个点的距离，i离谁最近则属于哪个k，最终得到k个族
3. 计算每个族的质心(x求个平均，y求和平均)，得到新的k个点
4. 重复2,3直到 k个族中的元素基本不再变化

![726445aa85e5b2fe4d1072eff1085b47](2024-01-24-近似临近搜索/726445aa85e5b2fe4d1072eff1085b47.png)
...
> 感觉和k-means一样 也是划分族 一个族构建一个倒排

* 倒排文件索引Ivf

  * 将向量数据集划分为M 个子集（簇），每个簇都有一个聚类中心向量a。通过构建倒排表，将聚类中心向量 a 与属于该簇其他向量进行关联。

  * 搜索时，先根据查询向量q找到与之最相似聚类中心向量a，在该聚类中心a对应倒排表中查找更接近q的具体向量p。

* 两级索引结构极大地減少搜索计算量，提高搜索效率。
* 需要具备一定数据后才开始训练（热启），DB 拥有一定量数据后需重建 IVF 索引表。

![fb4bce5e2502e8a3823aae3f4ed720ba](2024-01-24-近似临近搜索/fb4bce5e2502e8a3823aae3f4ed720ba.png)

**构建**

* 图中A点外围的圈就是A这个点覆盖的区域，覆盖了x2,x3,x4，所以建立倒排包含x2,x3,x4

 ![99dce406171decfe941ef17016d7eec2](2024-01-24-近似临近搜索/99dce406171decfe941ef17016d7eec2.png)
