...
//不再跟踪某文件
git rm --cached <文件名>
git rm --cached test/test.html

//不再跟踪某文件夹
git rm --cached <文件夹名> -r
git rm --cached test -r
```

* 只把文件从 Git 仓库中删除（即从暂存区域移除），但仍然保留在当前工作目录中。  即让文件保留在磁盘，但**Git不再继续跟踪**。  

**`git reset HEAD`和`git rm --cached`的区别**

* git rm -- cached 是从stage（暂存区） 里面删除文件，当你提交（commit）之后文件就不再被跟踪了。
  * **该文件不再被跟踪**
* git reset HEAD  : 回退暂存区里的文件（还原为HEAD commit里面该文件的状态），会撤销从上一次提交（commit）之后的一些操作。
  * **该文件仍然被跟踪，只是这次的修改没有加入暂存区**
* 如果是对于新增文件，这两个操作是等效的。
